...
/**
 * JUnit test for {@link SepaPnkIterator2}.
 *
 * @author Alistair A. Israel
 */
public final class SepaPnkIterator2Test extends PnkGeneratorTestBase {
    ...
    public void testGeneratePermutations() {
        testGenerateP53(new SepaPnkIterator2.Factory(5, 3));
    }
        ...
        for (int n = 2; n < 9; ++n) {
            for (int k = 1; k <= n; ++k) {
                final FactoradicNKPermutationsGenerator factoradic = new FactoradicNKPermutationsGenerator(
                        n, k);
                final Iterator<int[]> sepa = new SepaPnkIterator2(n, k);
                int i = 0;
                while (sepa.hasNext()) {
